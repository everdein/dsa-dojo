...
export const detectCycleLesson = {
  ...
  code: {
    ...
    lines: [
      { number: 9, text: "export function hasCycle(head) {", steps: ["function"] },
      { number: 10, text: "  validateLinkedListNode(head);", steps: ["initialize-slow", "initialize-fast"] },
      { number: 11, text: "  let slow = head;", steps: ["initialize-slow"] },
      { number: 12, text: "  let fast = head;", steps: ["initialize-fast"] },
      { number: 2, text: "", steps: ["guard"] },
      { number: 14, text: "  while (fast !== null && fast.next !== null) {", steps: ["guard"] },
      { number: 15, text: "    validateLinkedListNode(fast.next);", steps: ["guard"] },
      { number: 17, text: "    slow = slow.next;", steps: ["advance-slow"] },
      { number: 18, text: "    fast = fast.next.next;", steps: ["advance-fast"] },
      { number: 19, text: "    validateLinkedListNode(slow);", steps: ["advance-slow"] },
      { number: 20, text: "    validateLinkedListNode(fast);", steps: ["advance-fast"] },
      { number: 2, text: "", steps: ["compare"] },
      { number: 22, text: "    if (slow === fast) {", steps: ["compare"] },
      { number: 23, text: "      return true;", steps: ["return-true"] },
      { number: 24, text: "    }", steps: ["compare"] },
      { number: 25, text: "  }", steps: ["guard"] },
      { number: 2, text: "", steps: ["return-false"] },
      { number: 27, text: "  return false;", steps: ["return-false"] },
      { number: 28, text: "}", steps: ["function"] }
    ]
  },
  stats: [
    {
