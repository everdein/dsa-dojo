...
export const moveZerosLesson = {
  ...
  code: {
    ...
    lines: [
      { number: 15, text: "export function moveZeros(values) {", steps: ["function"] },
      { number: 16, text: "  validateMoveZerosInput(values);", steps: ["initialize"] },
      { number: 18, text: "  const result = [...values];", steps: ["initialize"] },
      { number: 19, text: "  let write = 0;", steps: ["initialize"] },
      { number: 21, text: "  for (let read = 0; read < result.length; read += 1) {", steps: ["inspect"] },
      { number: 22, text: "    if (result[read] === 0) continue;", steps: ["inspect"] },
      { number: 24, text: "    if (read !== write) {", steps: ["swap"] },
      { number: 25, text: "      const readValue = result[read];", steps: ["swap"] },
      { number: 26, text: "      result[read] = result[write];", steps: ["swap"] },
      { number: 27, text: "      result[write] = readValue;", steps: ["swap"] },
      { number: 28, text: "    }", steps: ["swap"] },
      { number: 29, text: "    write += 1;", steps: ["advance-write"] },
      { number: 30, text: "  }", steps: ["inspect"] },
      { number: 32, text: "  return result;", steps: ["return"] },
