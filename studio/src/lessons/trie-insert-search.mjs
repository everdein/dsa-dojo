...
export const trieInsertSearchLesson = {
  ...
  code: {
    ...
    lines: [
      ...
      { number: 34, text: "    for (const character of Array.from(normalized)) {", steps: ["insert-word"] },
      { number: 35, text: "      if (!node.children.has(character)) {", steps: ["follow-or-create"] },
      { number: 36, text: "        node.children.set(character, createTrieNode(`node-${nextId}`, character, character));", steps: ["follow-or-create"] },
      ...
      { number: 43, text: "  }", steps: ["insert-word"] },
      { number: 44, text: "  return root;", steps: ["initialize"] },
      { number: 47, text: "export function searchTrie(words, query) {", steps: ["search-query"] },
      { number: 48, text: "  validateTrieInput(words, query);", steps: ["search-query"] },
      { number: 49, text: "  const root = createTrie(words);", steps: ["search-query"] },
      { number: 50, text: "  const normalizedQuery = normalizeTrieWord(query);", steps: ["search-query"] },
      { number: 51, text: "  let node = root;", steps: ["search-query"] },
      { number: 52, text: "  for (const character of Array.from(normalizedQuery)) {", steps: ["search-query"] },
      { number: 53, text: "    const next = node.children.get(character);", steps: ["follow-edge"] },
      { number: 54, text: "    if (!next) return { found: false, isPrefix: false, normalizedQuery };", steps: ["return-missing"] },
      { number: 55, text: "    node = next;", steps: ["follow-edge"] },
      { number: 56, text: "  }", steps: ["search-query"] },
      { number: 57, text: "  return {", steps: ["return-found", "return-prefix"] },
      { number: 58, text: "    found: node.terminal,", steps: ["return-found"] },
      { number: 59, text: "    isPrefix: node.children.size > 0 || node.terminal,", steps: ["return-prefix"] },
      { number: 60, text: "    normalizedQuery", steps: ["return-found", "return-prefix"] },
      { number: 61, text: "  };", steps: ["return-found", "return-prefix"] },
      { number: 62, text: "}", steps: ["search-query"] }
    ]
  },
  stats: [
    {
