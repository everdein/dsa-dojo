...
export const rotateMatrixLesson = {
  ...
  code: {
    ...
    lines: [
      { number: 24, text: "export function rotateMatrix(matrix) {", steps: ["function"] },
      { number: 25, text: "  validateSquareMatrix(matrix);", steps: ["copy"] },
      { number: 26, text: "  const rotated = matrix.map((row) => [...row]);", steps: ["copy"] },
      { number: 27, text: "", steps: ["copy"] },
      { number: 28, text: "  for (let row = 0; row < rotated.length; row += 1) {", steps: ["transpose-loop"] },
      { number: 29, text: "    for (let column = row + 1; column < rotated.length; column += 1) {", steps: ["transpose-loop"] },
      { number: 30, text: "      [rotated[row][column], rotated[column][row]] = [", steps: ["transpose-swap"] },
      { number: 31, text: "        rotated[column][row], rotated[row][column]", steps: ["transpose-swap"] },
      { number: 32, text: "      ];", steps: ["transpose-swap"] },
      { number: 33, text: "    }", steps: ["transpose-loop"] },
      { number: 34, text: "  }", steps: ["transpose-loop"] },
      { number: 35, text: "", steps: ["row-loop"] },
      { number: 36, text: "  for (const row of rotated) row.reverse();", steps: ["row-loop", "reverse-row"] },
      { number: 37, text: "  return rotated;", steps: ["return"] },
