...
export const findDuplicatesLesson = {
  ...
  code: {
    ...
    lines: [
      { number: 15, text: "export function findDuplicates(values) {", steps: ["function"] },
      { number: 16, text: "  validateFindDuplicatesInput(values);", steps: ["initialize-sets"] },
      { number: 17, text: "", steps: ["initialize-sets"] },
      { number: 18, text: "  const seen = new Set();", steps: ["initialize-sets"] },
      { number: 19, text: "  const emitted = new Set();", steps: ["initialize-sets"] },
      { number: 20, text: "  const duplicates = [];", steps: ["initialize-sets"] },
      { number: 21, text: "", steps: ["scan-value"] },
      { number: 22, text: "  for (const value of values) {", steps: ["scan-value"] },
      { number: 23, text: "    if (seen.has(value)) {", steps: ["check-seen"] },
      { number: 24, text: "      if (!emitted.has(value)) {", steps: ["check-emitted"] },
      { number: 25, text: "        duplicates.push(canonicalResultValue(value));", steps: ["append-duplicate"] },
      { number: 26, text: "        emitted.add(value);", steps: ["mark-emitted"] },
      { number: 27, text: "      }", steps: ["check-emitted"] },
      { number: 28, text: "    } else {", steps: ["check-seen"] },
      { number: 29, text: "      seen.add(value);", steps: ["record-seen"] },
      { number: 30, text: "    }", steps: ["check-seen"] },
      { number: 31, text: "  }", steps: ["scan-value"] },
      { number: 32, text: "", steps: ["return-duplicates"] },
      { number: 33, text: "  return duplicates;", steps: ["return-duplicates"] },
