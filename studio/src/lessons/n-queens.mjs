...
export const nQueensLesson = {
  ...
  code: {
    ...
    lines: [
      ...
      { number: 19, text: "    if (row === size) {", steps: ["record"] },
      { number: 20, text: "      solutions.push([...placement]);", steps: ["record"] },
      { number: 21, text: "      return;", steps: ["record"] },
      { number: 22, text: "    }", steps: ["record"] },
      { number: 23, text: "    for (let column = 0; column < size; column += 1) {", steps: ["try-column"] },
      { number: 24, text: "      const fallingKey = row - column;", steps: ["check-safe"] },
      { number: 25, text: "      const risingKey = row + column;", steps: ["check-safe"] },
      { number: 26, text: "      if (columns.has(column) || falling.has(fallingKey) || rising.has(risingKey)) continue;", steps: ["check-safe", "skip-unsafe"] },
