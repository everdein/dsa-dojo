...
export const connectedComponentsLesson = {
  ...
  code: {
    ...
    lines: [
      ...
      { number: 7, text: "  const adjacency = buildAdjacency(nodes, edges);", steps: ["initialize"] },
      { number: 8, text: "  const visited = new Set();", steps: ["initialize"] },
      { number: 9, text: "  const components = [];", steps: ["initialize"] },
      { number: 10, text: "  for (const start of nodes) {", steps: ["scan-start"] },
      { number: 11, text: "    if (visited.has(start)) continue;", steps: ["scan-start"] },
      { number: 12, text: "    const component = [];", steps: ["start-bfs"] },
      { number: 13, text: "    const queue = [start];", steps: ["start-bfs"] },
      { number: 14, text: "    visited.add(start);", steps: ["start-bfs"] },
      { number: 15, text: "    while (head < queue.length) {", steps: ["dequeue"] },
      { number: 16, text: "      const node = queue[head++];", steps: ["dequeue"] },
      { number: 17, text: "      component.push(node);", steps: ["record"] },
      { number: 18, text: "      for (const neighbor of adjacency.get(node)) {", steps: ["scan-neighbors"] },
      { number: 19, text: "        if (visited.has(neighbor.node)) continue;", steps: ["scan-neighbors"] },
      { number: 20, text: "        visited.add(neighbor.node);", steps: ["enqueue"] },
      { number: 21, text: "        queue.push(neighbor.node);", steps: ["enqueue"] },
      { number: 22, text: "      }", steps: ["scan-neighbors"] },
      { number: 23, text: "    }", steps: ["dequeue"] },
      { number: 24, text: "    components.push(component);", steps: ["start-bfs"] },
      { number: 25, text: "  }", steps: ["scan-start"] },
      { number: 26, text: "  return components;", steps: ["return"] },
